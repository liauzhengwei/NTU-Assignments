...
void recursiveReverse(Queue *q){
    //prob wrong method as no recursion used.
...
int palindrome(char *word){
	...
	//1. Divide the queue into 2.
	//2.dequeue first part into stack
	//3. pop first element of second part if odd
	//4. compare first and second part

	//return 0 if palindrome, -1 if not
	// ignore caps and space

	//not sure if correct solution, issues with the return part
	int i;
	Queue q1;
	q1.ll.size=0;
	q1.ll.head=NULL;
...
int balanced(char *expression){
    ...
    //1. if odd, not balanced
    //2. Divide by 2 and copy steps 2,3,4 of qn3

    //wrongly get correct results, only the odd part works
    int i;
	Queue q;
	q.ll.size=0;
	q.ll.head=NULL;
