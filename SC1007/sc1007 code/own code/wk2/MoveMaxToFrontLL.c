...
int moveMaxToFront(ListNode **ptrHead)
{
    ...
    while (cur!=NULL)
    {
        if((cur->item) > a)
        {
            a = cur->item;		//finds the largest value
            c = count-1;		//finds the index of the largest value
        }
        cur=cur->next;
        count++;
    }
    ...
    removeNodeb(ptrHead, c);
    printf("adawg\n");
    insertNodeb(ptrHead,0,a);
}

int removeNodeb(ListNode **ptrHead, int index){
	ListNode *pre, *cur;
...
int insertNodeb(ListNode **ptrHead, int index, int value){
    ListNode *pre, *cur;
