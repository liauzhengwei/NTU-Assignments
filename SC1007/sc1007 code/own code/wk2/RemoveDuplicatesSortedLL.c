...
void removeDuplicatesSortedLL(LinkedList *ll)
{
	...
	while(temp!=NULL)
    {
        if((cur->item) == (temp->item))	//since arranged in ascending order, duplicates are next to each other on the linked list
        {
            cur=cur->next;
            removeNode(ll, count);
            temp=cur->next;
        ...
        {	
            count+=1;			//count only moves when the numbers next to each other are different so no removal done
            cur=cur->next;
            temp=cur->next;
        }
        printf("adagcfewwa\n");
